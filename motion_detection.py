...
def detectMotion(frame, colored):
    global baseFrame
    ...
    thresh = cv.threshold(frameDelta, 55, 255, cv.THRESH_BINARY)[1]
    ...
    for c in contours:
        ...
        if cv.contourArea(c) < getThreshold():
            continue
        # if the contour area is larger, we’ll draw the bounding box 
        # surrounding the foreground and motion region
        cv.rectangle(colored, (x, y), (x + w, y + h), (0, 255, 0), 2)
        cv.putText(colored, "Status: {}".format('Movement'), (10, frame.shape[0] - 10), cv.FONT_HERSHEY_SIMPLEX,
            1, (0, 0, 255), 2)
